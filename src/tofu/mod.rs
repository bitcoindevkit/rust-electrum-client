...
pub trait TofuStore: Send + Sync + Debug {
    ...
    fn get_certificate(&self, host: &str) -> Result<Option<Vec<u8>>, Box<dyn std::error::Error + Send + Sync>>;
    ...
    fn set_certificate(&self, host: &str, cert: Vec<u8>) -> Result<(), Box<dyn std::error::Error + Send + Sync>>;
}
